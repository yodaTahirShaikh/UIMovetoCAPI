...
async function processData(ldap, forceMigrate = false) {
    if (!forceMigrate) {
        ...
        const resultcsvData = await readSingleCsv('result', fileName);
        for (let data of resultcsvData) {
            if (!data['AppID'] || !data['AppName'] || !data['PhoneNumber'] || !data['WabaId'] || !data['PhoneId']) {
                return "Invalid data";
            }

            if (data['Status'] === '400' && data['Response'].message === 'Given phone number does not match with app details record') {
                ...
                try {
                    ...
                    if (error.response.status === 400) {
                        ...
                        logger(`${data['PhoneNumber']} | status=${error.response.status} | ${error.response.data} || forceMigrate=${forceMigrate}`);
                    };
