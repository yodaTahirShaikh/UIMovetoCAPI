...
function readSingleCsv(directory, filename) {
    const data = [];
    const files = fs.readdirSync(directory).filter(file => file.startsWith(filename));
    const fileName = files[0] || '';
    if (fileName === '') {
        return Promise.resolve([]);
    }
...
module.exports = { readCsv, readSingleCsv }